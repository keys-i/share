import { getMimeType } from "@/lib/file-utils";
import type { FileContent } from "@/lib/types/github";

interface ImageViewerProps {
  file: FileContent;
}

export function ImageViewer({ file }: ImageViewerProps) {
  const mimeType = getMimeType(file.name);
  const src = file.encoding === "base64" ? `data:${mimeType};base64,${file.content}` : file.download_url;

  return (
    <div className="flex items-center justify-center p-8 min-h-[400px] bg-[url('data:image/svg+xml;base64,')]">
      <img src={src} alt={file.name} className="max-w-full max-h-[600px] object-contain shadow-lg rounded" />
    </div>
  );
}
