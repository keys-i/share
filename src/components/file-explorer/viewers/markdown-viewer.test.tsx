...
describe("MarkdownViewer", () => {
  ...
  it("should render absolute image URLs as-is", () => {
    const file = createMockFile({
      content: Buffer.from("![Logo](https://example.com/logo.png)").toString("base64"),
    });
  ...
  it("should render GFM tables", () => {
    const file = createMockFile({
      content: Buffer.from("| Header 1 | Header 2 |\n|----------|----------|\n| Cell 1 | Cell 2 |").toString("base64"),
    });
  ...
  it("should have prose styling classes", () => {
    ...
    const { container } = render(<MarkdownViewer file={file} {...defaultProps} />);
