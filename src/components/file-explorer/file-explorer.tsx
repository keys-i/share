...
export function FileExplorer({
  ...
  const handleToggleExpand = async (node: TreeNode) => {
    ...
    if (isExpanded) {
      ...
      if (onLoadChildren && node.type === "tree" && (!node.children || node.children.length === 0)) {
  ...
  return (
    ...
    >
      ...
      {/* Branch selector */}
      <div className="flex items-center gap-2 px-3 py-2 border-b shrink-0">
        <Select value={branch} onValueChange={(value) => value && onBranchChange?.(value)} disabled={isTreeLoading}>
      ...
      <ScrollArea className="flex-1 min-h-0">
        {(isTreeLoading || sortedTree.length === 0) &&
          ...
          [62, 45, 71, 38, 55, 68, 42, 74, 51, 60, 47, 66, 53, 40, 72, 58, 44, 69, 36, 63].map((width) => (
            <div key={`tree-skeleton-w${width}`} className="px-2 py-1">
              <Skeleton className="h-6 bg-muted-foreground/20 w-full" style={{ width: `${width}%` }} />
            </div>
          ))}
        {!isTreeLoading && sortedTree.length > 0 && (
