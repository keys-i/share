...
export function TreeItem({
  ...
  const handleClick = async () => {
    if (isFolder) {
      if (onToggleExpand) {
        ...
        if (willOpen && onLoadChildren && (!node.children || node.children.length === 0)) {
  ...
  if (searchQuery) {
    ...
    const hasMatchingChild = isFolder && node.children && hasMatchingDescendant(node, searchQuery);
      ...
      >
        ...
        {isFolder ? (
          isLoading ? (
            ...
            <Folder className="size-4 shrink-0 text-blue-400" fill="currentColor" />
          )
        ) : (
      ...
      {isFolder && isOpen && (
        <div>
          {isLoading ? (
            ...
            <div className="py-2 px-2 text-sm text-destructive" style={{ paddingLeft: `${(depth + 1) * 16 + 8}px` }}>
