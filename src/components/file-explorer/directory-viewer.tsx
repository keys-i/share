...
export function DirectoryViewer({ directory, onNavigate, onFileSelect, onHover }: DirectoryViewerProps) {
      ...
      <table className="w-full table-fixed">
        <thead>
          <tr className="border-b text-left text-sm text-muted-foreground">
            ...
            <th className="px-4 py-2 font-medium text-right w-1/6">Size</th>
          </tr>
        </thead>
        <tbody>
            ...
            >
              <td className="px-4 py-2">
                <div className="flex items-center gap-2">
                  <FolderUp className="size-4 text-muted-foreground" fill="currentColor" />
                  <span>..</span>
                </div>
              </td>
            ...
            >
              <td className="px-4 py-2">
                <div className="flex items-center gap-2 min-w-0">
                  {entry.type === "tree" ? (
                    <Folder className="size-4 text-blue-400 shrink-0" fill="currentColor" />
                  ) : (
                    <File className="size-4 text-muted-foreground shrink-0" />
                  )}
                  <span className="hover:text-blue-500 hover:underline truncate">{entry.name}</span>
                </div>
              </td>
              <td className="px-4 py-2 text-right text-muted-foreground text-sm whitespace-nowrap">
                {entry.type === "blob" && entry.size != null ? formatFileSize(entry.size) : ""}
              </td>
            </tr>
          ))}
        </tbody>
