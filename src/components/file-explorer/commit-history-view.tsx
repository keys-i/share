import { ChevronLeft, ChevronRight, Code, Copy, GitCommitHorizontal, X } from "lucide-react";
 ...
 */
function groupCommitsByDate(commits: CommitHistoryEntry[]): Map<string, CommitHistoryEntry[]> {
    ...
    <div>
      ...
      <div className="sticky top-0! md:top-4 z-10 bg-background rounded-t-lg">
        ...
        <div className="px-4 py-2 border-b bg-muted/30 text-sm">
          {isRoot ? (
            <div className="flex items-center gap-2">
              <span className="px-2 py-0.5 bg-muted rounded text-xs font-mono">{branch}</span>
            </div>
          ) : (
            <div className="flex items-center gap-1 min-w-0">
              <span className="shrink-0">History for</span>
              <span className="text-blue-500 hover:underline shrink-0">{repoName}</span>
              <span className="shrink-0">/</span>
              <span className="font-medium truncate">{path}</span>
              <span className="ml-2 shrink-0">on</span>
              <span className="px-2 py-0.5 bg-muted rounded text-xs font-mono shrink-0">{branch}</span>
            </div>
          )}
        </div>
                ...
                {commits.map((commit: CommitHistoryEntry, index: number) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-medium text-sm truncate">{commit.message.split("\n")[0]}</div>
                      <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                        {commit.authorAvatarUrl ? (
                          <img src={commit.authorAvatarUrl} alt={commit.author} className="size-4 rounded-full" />
                        ) : (
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <Button variant="ghost" size="icon-xs" aria-label="View code at this commit">
