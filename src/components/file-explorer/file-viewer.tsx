import { Check, Copy, Download, History, LinkIcon, MoreHorizontal, PanelLeft, WrapText } from "lucide-react";
...
import { countLines, decodeContent, downloadFile, getFileType } from "@/lib/file-utils";
import { formatDate, formatFileSize } from "@/lib/format";
import type { CommitHistoryData, CommitInfo, DirectoryContent, FileContent } from "@/lib/types/github";
...
export function FileViewer({
  ...
  if (showHistory) {
    return (
      <div data-slot="file-viewer" className={cn("border rounded-lg bg-background", className)}>
        <CommitHistoryView
  ...
  const decodedContent = isTextBased && file ? decodeContent(file.content, file.encoding) : "";
    ...
    <div data-slot="file-viewer" className={cn("border rounded-lg bg-background", className)}>
      ...
      <div className="sticky top-2 lg:top-4 z-20 bg-background rounded-t-lg before:absolute before:-left-px before:-right-px before:bottom-full before:h-2 lg:before:h-4 before:bg-muted">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b gap-2">
          ...
          <Breadcrumb repoName={repoName} path={currentPath} onNavigate={onNavigate} />
          <DropdownMenu>
            <DropdownMenuTrigger
            ...
            <DropdownMenuContent align="end" className="w-48">
              {file && (
                <>
                  <DropdownMenuItem onClick={() => downloadFile(file)} disabled={isLoading}>
              ...
              {file && (
                <>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem disabled={isLoading} onClick={() => setWrapText(!wrapText)}>
        ...
        {isLoading ? (
          ...
          <div className="flex items-center gap-3 px-4 py-2 border-b bg-muted/30">
            {displayCommit.authorUrl ? (
              <a href={displayCommit.authorUrl} target="_blank" rel="noopener noreferrer" className="shrink-0">
            ...
            <div className="flex-1 min-w-0 flex items-center">
              <span className="font-medium text-sm shrink-0">{displayCommit.author}</span>
              <span className="text-muted-foreground text-sm ml-2 truncate">
                {displayCommit.message.split("\n")[0]}
              </span>
            </div>
            <div className="flex items-center gap-2 text-xs text-muted-foreground shrink-0">
              <span className="font-mono">{displayCommit.sha.substring(0, 7)}</span>
              <span>·</span>
              ...
              >
                ...
                <span>
                  {isRoot && totalCommits !== undefined ? `${totalCommits.toLocaleString()} commits` : "History"}
                </span>
              </Button>
            </div>
          </div>
        ...
        {file && (
          <div className="flex items-center justify-between px-4 py-2 border-b">
              ...
              {isMarkdown ? (
                <>
                  <Button
                    ...
                    className={cn(showMarkdownPreview && "bg-accent text-accent-foreground")}
                  >
                    Preview
                  </Button>
                  <Button
                ...
                <>
                  <div className={cn("px-3 py-1.5 text-sm font-medium rounded-md", "bg-muted text-muted-foreground")}>
                    Code
                  </div>
                ...
                <span className="px-3 py-1.5 text-sm font-medium text-muted-foreground">Preview</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              {isTextBased ? (
                <span>
                  {lineCount} lines ({countLines(decodedContent)} loc) · {formatFileSize(file.size)}
                </span>
              ) : (
                ...
                {isTextBased && (
                  <>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        await navigator.clipboard.writeText(decodeContent(file.content, file.encoding));
                      }}
                    >
                      <Copy className="size-4" />
                    </Button>
      ...
      {isLoading ? (
        ...
        <DirectoryViewer directory={directory} onNavigate={onNavigate} onFileSelect={onFileSelect} onHover={onHover} />
      ) : file ? (
        <React.Suspense fallback={<output className="block p-6 text-muted-foreground">Loading file…</output>}>
          <FileContentRenderer
            file={file}
            wrapText={wrapText}
      ...
      <table className="w-full table-fixed">
        <thead>
          <tr className="border-b text-left text-sm text-muted-foreground">
            ...
            <th className="px-4 py-2 font-medium text-right w-1/6">Last commit date</th>
          </tr>
        </thead>
        <tbody>
          {SKELETON_ROWS.map((n) => (
