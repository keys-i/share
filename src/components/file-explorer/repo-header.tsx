    ...
    <Popover>
      ...
      <PopoverContent align="end" className="w-80 p-3">
        <PopoverHeader className="gap-2">
          <PopoverTitle>Read-only repository access</PopoverTitle>
          <PopoverDescription className="text-xs leading-relaxed">
            The repository owner has shared this repository for seven days. They can revoke the underlying token in
            GitHub to stop access.
          </PopoverDescription>
        </PopoverHeader>
        <a
...
export function RepoHeader({
  ...
  const { data: user } = useQuery({
    ...
    queryFn: async () => await getUser(repo.owner, accessToken?.startsWith("share:") ? undefined : accessToken),
  });
  ...
  return (
    <div className={cn("flex flex-col gap-3 p-3 md:p-4 border-b bg-background shadow-lg z-30", className)}>
      {/* Top row: Avatar, name, visibility, clone button */}
      <div className="flex items-center gap-3">
        {(repo.avatarUrl || user?.avatar_url) && (
          <a href={`https://github.com/${repo.owner}`} target="_blank" rel="noopener noreferrer">
            <img src={repo.avatarUrl || user?.avatar_url} alt={repo.owner} className="size-8 md:size-10 rounded-full" />
          </a>
        )}
          ...
          <Popover>
            ...
            <PopoverContent align="end" className="w-80 p-0">
              <div className="p-3 border-b">
                ...
                {/* HTTPS */}
                <div className="mb-3">
                  <div className="text-xs text-muted-foreground mb-1">HTTPS</div>
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <Button variant="ghost" size="icon-sm" onClick={() => handleCopy(httpsUrl, "https")}>
                      {copied === "https" ? <Check className="size-4 text-green-500" /> : <Copy className="size-4" />}
                    </Button>
                  </div>
                </div>
              </div>
      ...
      {/* Description */}
      {repo.description && <p className="text-sm text-muted-foreground hidden sm:block">{repo.description}</p>}
        ...
        <div className="flex flex-col gap-2">
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            <Link
              to="/$"
              params={{
                _splat: `${repoPath}/blob/${repo.defaultBranch}/README.md`,
              }}
            >
              <Button
                variant="outline"
            ...
            {repo.watchers !== undefined && (
              ...
              >
                ...
                <span className="font-medium">{formatCount(repo.watchers)}</span>
            ...
            {branchCount !== undefined && branchCount > 0 && (
              ...
              >
                ...
                <span className="hidden sm:inline">{branchCount === 1 ? "branch" : "branches"}</span>
              </Button>
            )}
          ...
          {repo.topics && repo.topics.length > 0 && (
            <div className="hidden md:flex flex-wrap gap-1.5">
              ...
              {repo.topics.length > 5 && (
                <span className="px-2 py-0.5 text-xs text-muted-foreground">+{repo.topics.length - 5} more</span>
              )}
            </div>
          )}
        </div>
          ...
          {contributors && contributors.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              {totalContributors && totalContributors > 5 && (
                <span className="text-xs text-muted-foreground">+{totalContributors - 5} contributors</span>
              )}
            </div>
          )}
 ...
 */
export function MinimalRepoHeader({ owner, repo, className }: MinimalRepoHeaderProps) {
  return (
    <div className={cn("flex flex-col gap-3 p-3 md:p-4 border-b bg-background shadow-lg z-30", className)}>
      <div className="flex items-center gap-3">
        <a href={`https://github.com/${owner}`} target="_blank" rel="noopener noreferrer">
          <img src={`https://github.com/${owner}.png?size=80`} alt={owner} className="size-8 md:size-10 rounded-full" />
        </a>
        <div className="flex items-center gap-2 min-w-0 flex-1">
          <a
