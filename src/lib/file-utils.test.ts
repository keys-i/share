...
it("keeps relative Markdown assets inside the shared repository proxy", () => {
  const downloadUrl =
    "/api/github/keys/7m4k-p9rx-2w6c-8h3n/repos/keys/demo/contents/docs/README.md?ref=feat%2Fdemo&raw=1";
  expect(resolveRelativeUrl("../media/my image.png", "docs/README.md", "keys", "demo", "feat/demo", downloadUrl)).toBe(
    "/api/github/keys/7m4k-p9rx-2w6c-8h3n/repos/keys/demo/contents/media/my%20image.png?ref=feat%2Fdemo&raw=1",
  );
  expect(
    resolveRelativeUrl(
      "../media/my%20image.png?raw=1#logo",
      "docs/README.md",
      "keys",
      "demo",
      "feat/demo",
    ...
    "/api/github/keys/7m4k-p9rx-2w6c-8h3n/repos/keys/demo/contents/media/my%20image.png?ref=feat%2Fdemo&raw=1#logo",
  );
  expect(
    resolveRelativeUrl(
      "../media/my%20image.png?raw=1#logo",
      ...
      "demo",
      "feat/demo",
      `https://api.share.keysi.dev${downloadUrl}`,
    ),
  ).toBe(
    "https://api.share.keysi.dev/api/github/keys/7m4k-p9rx-2w6c-8h3n/repos/keys/demo/contents/media/my%20image.png?ref=feat%2Fdemo&raw=1#logo",
  );
});
...
describe("decodeContent", () => {
  ...
  it("should handle invalid base64 gracefully", () => {
    ...
    expect(decodeContent("not valid base64!!!", "base64")).toBe("not valid base64!!!");
  });
});
...
describe("resolveRelativeUrl", () => {
  ...
  it("should return absolute URLs unchanged", () => {
    expect(resolveRelativeUrl("https://example.com/image.png", "README.md", owner, repo, branch)).toBe(
      "https://example.com/image.png",
    );

    expect(resolveRelativeUrl("http://example.com/image.png", "README.md", owner, repo, branch)).toBe(
  ...
  it("should return data URLs unchanged", () => {
    ...
    expect(resolveRelativeUrl(dataUrl, "README.md", owner, repo, branch)).toBe(dataUrl);
  });

  it("should resolve relative paths with ./", () => {
    expect(resolveRelativeUrl("./image.png", "docs/README.md", owner, repo, branch)).toBe(
  ...
  it("should resolve absolute paths from repo root", () => {
    expect(resolveRelativeUrl("/assets/image.png", "docs/README.md", owner, repo, branch)).toBe(
      "https://raw.githubusercontent.com/test-owner/test-repo/main/assets/image.png",
    );
  });

  it("should resolve parent directory paths", () => {
    expect(resolveRelativeUrl("../image.png", "docs/guide/README.md", owner, repo, branch)).toBe(
      "https://raw.githubusercontent.com/test-owner/test-repo/main/docs/image.png",
    );

    expect(resolveRelativeUrl("../../image.png", "docs/guide/deep/README.md", owner, repo, branch)).toBe(
      "https://raw.githubusercontent.com/test-owner/test-repo/main/docs/image.png",
    );
  });

  it("should handle files at root level", () => {
    expect(resolveRelativeUrl("image.png", "README.md", owner, repo, branch)).toBe(
