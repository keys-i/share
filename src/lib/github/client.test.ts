...
import { createGitHubClient, withTimeout } from "./client";

it("sends shared repository reads to the API without a GitHub token", async () => {
  const share = "keys/7m4k-p9rx-2w6c-8h3n";
  server.use(
    http.get(`https://api.share.keysi.dev/api/github/${share}/repos/keys/demo`, ({ request }) => {
      expect(request.headers.get("authorization")).toBeNull();
      return HttpResponse.json({ name: "demo" });
    }),
  );
  const result = await createGitHubClient(`share:${share}`).request("GET /repos/{owner}/{repo}", {
    owner: "keys",
    repo: "demo",
  });
...
describe("withTimeout", () => {
  ...
  it("should reject with timeout error when promise takes too long", async () => {
    ...
    await expect(withTimeout(slowPromise, 100)).rejects.toThrow("Request timed out after 100ms");
  });
