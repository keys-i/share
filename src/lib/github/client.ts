...
export function createGitHubClient(token?: string): Octokit {
  if (token?.startsWith("share:")) {
    return new Octokit({ baseUrl: `${SHARE_API_ORIGIN}/api/github/${token.slice(6)}` });
  }
  return new Octokit({
    auth: token,
  });
 ...
 */
export async function withTimeout<T>(promise: Promise<T>, timeoutMs: number = REQUEST_TIMEOUT): Promise<T> {
...
let rateLimitCallback: ((headers: Record<string, string | number | undefined>, token?: string) => void) | null = null;
...
export function registerRateLimitCallback(
  callback: (headers: Record<string, string | number | undefined>, token?: string) => void,
): void {
 ...
 */
export function notifyRateLimitHeaders(headers: Record<string, string | number | undefined>, token?: string): void {
 ...
 */
export function extractRateLimitHeaders(headers: Record<string, unknown>): Record<string, string | number | undefined> {
  return {
    "x-ratelimit-limit": headers["x-ratelimit-limit"] as number | undefined,
    "x-ratelimit-remaining": headers["x-ratelimit-remaining"] as number | undefined,
