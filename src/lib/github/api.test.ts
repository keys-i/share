...
import { mockCommit, mockContributors, mockFileContent, mockUser } from "@/test/mocks/handlers";
...
describe("getContributors", () => {
  it("should return contributors on success", async () => {
    const result = await getContributors("test-owner", "test-repo", "test-token");
  ...
  it("should parse total count from Link header", async () => {
    server.use(
      http.get("https://api.github.com/repos/:owner/:repo/contributors", ({ request }) => {
...
describe("getFileContent", () => {
  it("loads metadata for large shared media before streaming its preview", async () => {
    const share = "test-owner/7m4k-p9rx-2w6c-8h3n";
    const base = `https://api.share.keysi.dev/api/github/${share}/repos/test-owner/test-repo`;
    server.use(
      http.get(`${base}/contents/*`, ({ request }) => {
        expect(request.headers.get("accept")).toBe("application/vnd.github.object+json");
        return HttpResponse.json({ ...mockFileContent, name: "large.png", encoding: "none", content: "" });
      }),
      http.get(`${base}/commits`, () => HttpResponse.json([mockCommit])),
    );
    ...
    expect(result.content.download_url).toBe(`${base}/contents/large.png?ref=main&raw=1`);
  });

  it("uses the share proxy for raw files and downloads without sending a token", async () => {
    const share = "test-owner/7m4k-p9rx-2w6c-8h3n";
    const base = `https://api.share.keysi.dev/api/github/${share}/repos/test-owner/test-repo`;
    server.use(
      http.get(`${base}/contents/*`, ({ request }) => {
    ...
    const result = await getFileContent("test-owner", "test-repo", "my image.png", "feat/demo", 1024, `share:${share}`);
    expect(result.content.download_url).toBe(
      `https://api.share.keysi.dev/api/github/${share}/repos/test-owner/test-repo/contents/my%20image.png?ref=feat%2Fdemo&raw=1`,
    );
  });

  it("should return file content on success", async () => {
    const result = await getFileContent("test-owner", "test-repo", "README.md", "main", 1024, "test-token");
  ...
  it("should throw error for file too large", async () => {
    ...
    await expect(
      getFileContent("test-owner", "test-repo", "huge-file.bin", "main", hugeSize, "test-token"),
  ...
  it("should use raw format for large files (>1MB)", async () => {
    ...
    const result = await getFileContent("test-owner", "test-repo", "large-file.txt", "main", largeSize, "test-token");
  ...
  it("should throw error for file not found", async () => {
    ...
    await expect(
      getFileContent("test-owner", "test-repo", "nonexistent.txt", "main", 100, "test-token"),
  ...
  it("should throw error if path is a directory", async () => {
    ...
    await expect(getFileContent("test-owner", "test-repo", "src", "main", undefined, "test-token")).rejects.toThrow(
  ...
  it("should handle too_large error from GitHub", async () => {
    server.use(
      http.get("https://api.github.com/repos/:owner/:repo/contents/:path*", () => {
        return HttpResponse.json(
          {
            message: "This API returns blobs up to 1 MB in size. The requested blob is too_large to fetch via the API.",
          },
          { status: 403 },
        );
      }),
...
describe("getCommitHistory", () => {
  it("should return commit history on success", async () => {
    const result = await getCommitHistory("test-owner", "test-repo", "", "main", 1, 10, "test-token");
  ...
  it("should filter by path", async () => {
    server.use(
      http.get("https://api.github.com/repos/:owner/:repo/commits", ({ request }) => {
        ...
        if (path === "src/index.ts") {
          return HttpResponse.json([{ ...mockCommit, sha: "path-specific-commit" }]);
        }
    ...
    const result = await getCommitHistory("test-owner", "test-repo", "src/index.ts", "main", 1, 10, "test-token");
...
describe("getTotalCommitCount", () => {
  it("should return count from Link header", async () => {
    ...
    const result = await getTotalCommitCount("test-owner", "test-repo", "main", "test-token");
...
it("loads a directory with two requests regardless of its file count", async () => {
  let calls = 0;
  server.use(
    http.get("https://api.github.com/repos/test-owner/test-repo/contents/src", () => {
      calls++;
      return HttpResponse.json(
    ...
    http.get("https://api.github.com/repos/test-owner/test-repo/commits", () => {
      calls++;
      return HttpResponse.json([mockCommit]);
    }),
  );
  const result = await getDirectoryContentDirect("test-owner", "test-repo", "src", "main", "test-token");
