...
import { SHARE_API_ORIGIN } from "../shares";
import {
  createGitHubClient,
 ...
 */
export async function getBranches(owner: string, repo: string, token: string): Promise<BranchInfo[]> {
  ...
  try {
    ...
    if (error instanceof DOMException && error.name === "TimeoutError") {
      throw new RateLimitError("Request timed out. GitHub API may be rate limited. Please try again later.");
    }
 ...
 */
export async function getContributors(owner: string, repo: string, token: string): Promise<ContributorsResult> {
  const octokit = createGitHubClient(token);

  try {
    // First, get a count by requesting 1 per page and checking the Link header
    const countResult = await octokit.request("GET /repos/{owner}/{repo}/contributors", {
      owner,
      repo,
      per_page: 1,
      anon: "false",
...
export async function getDirectoryTree(
  ...
  try {
    ...
    if (path) {
      ...
      const parentResult = await octokit.request("GET /repos/{owner}/{repo}/git/trees/{tree_sha}", {
        owner,
        repo,
        tree_sha: branch,
      ...
      const targetItem = treeData.find(
        (item) => item.type === "tree" && (parentPath ? item.path === path : item.path === targetName),
      );
 ...
 */
export async function getRepoTree(owner: string, repo: string, branch: string, token: string): Promise<TreeNode[]> {
  const octokit = createGitHubClient(token);
  ...
  try {
    ...
    if (result.data.truncated) {
      console.warn("Tree was truncated due to size limits. Falling back to manual traversal.");
...
export async function getDirectoryContentDirect(
  ...
  try {
    ...
    try {
      const commitsResult = await octokit.request("GET /repos/{owner}/{repo}/commits", {
        owner,
        repo,
        path: path || undefined,
        sha: branch,
...
export async function getFileContent(
  ...
  const isLargeTextFile = isLargeFile && ["text", "markdown"].includes(getFileType(path));

  try {
    const contentResult = await octokit.request("GET /repos/{owner}/{repo}/contents/{path}", {
      owner,
      repo,
      path,
      ref: branch,
      headers: {
        ...GITHUB_API_HEADERS,
        ...(isLargeFile && {
          Accept: isLargeTextFile ? "application/vnd.github.raw+json" : "application/vnd.github.object+json",
        }),
      },
    });
    ...
    if (token.startsWith("share:")) {
      ...
      const query = new URLSearchParams({ ref: branch, raw: "1" });
      fileContent.download_url = `${SHARE_API_ORIGIN}/api/github/${token.slice(6)}/repos/${owner}/${repo}/contents/${encodedPath}?${query}`;
    }
    ...
    if (error instanceof Error) {
      if (error.message.includes("too_large")) {
        throw new Error("File is too large to display. Please download it directly.");
      }
 ...
 */
export async function getTotalCommitCount(owner: string, repo: string, branch: string, token: string): Promise<number> {
  const octokit = createGitHubClient(token);
