...
import { createGitHubClient, registerRateLimitCallback, unregisterRateLimitCallback } from "./client";
...
export function useRateLimit(token?: string): RateLimitContextType {
  ...
  React.useEffect(() => {
    const handleRateLimitHeaders = (headers: Record<string, string | number | undefined>, responseToken?: string) => {
      ...
      if (remaining !== undefined && limit !== undefined) {
        ...
        if (!Number.isNaN(remainingNum) && !Number.isNaN(limitNum)) {
          ...
          queryClient.setQueryData([RATE_LIMIT_QUERY_KEY, tokenRef.current || "anonymous"], newState);
        }
      }
    };
